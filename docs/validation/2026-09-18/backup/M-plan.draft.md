...
## 验收标准
- [ ] AC-1：带 `Idempotency-Key: k1` 首次提交合法订单 → 201，返回订单；库中 1 条且 `idempotency_key = k1`、`request_hash` 非空。证据：OrderApiIT，真实 Postgres 查库
- [ ] AC-2：同一 `k1`、同一请求体（含 `10` 与 `10.00` 这种等值写法）再次提交 → 201，响应 id / orderNo / amount 与首次相同，带 `Idempotent-Replayed: true`；库中仍 1 条。证据：OrderApiIT，真实 Postgres 计数
- [ ] AC-3：同一 `k2`、同一请求体 8 个线程并发提交 → 全部 201 且 id 相同；库中该键仅 1 条。证据：OrderApiIT 并发用例，真实 Postgres 唯一约束生效
- [ ] AC-4：同一 `k1`、请求体不同（如 quantity 改变）→ 422 `IDEMPOTENCY_KEY_REUSED`；库中仍 1 条且金额为首次值。证据：OrderApiIT，真实 Postgres
- [ ] AC-5：不带请求头 → 行为不变：正常 201；重复 orderNo 409 且不落库；不同幂等键但 orderNo 重复 → 409。证据：OrderApiIT（现有 3 个用例保持通过 + 新增不同键同 orderNo 用例）
- [ ] AC-6：`Idempotency-Key` 为空白或超过 64 字符 → 400 `INVALID_IDEMPOTENCY_KEY`，不调用创建逻辑。证据：WebMvcTest 正反例（合法键放行、非法键 400）

## 任务（仅 L 档）
- [ ] T1 契约与类型：`Order` 新列与唯一约束、`OrderRepository.findByIdempotencyKey`、`IdempotencyKeyReusedException`、Controller 请求头参数签名；以 `./mvnw -q test` 编译通过验证
- [ ] T2 首次创建与顺序重放（对应 AC-1、AC-2；依赖 T1）
- [ ] T3 同键不同体返回 422（对应 AC-4；依赖 T2）
- [ ] T4 并发同键只落一条（对应 AC-3；依赖 T2）
- [ ] T5 请求头校验与无键兼容、不同键同 orderNo 409（对应 AC-5、AC-6；依赖 T1）
