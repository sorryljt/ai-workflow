...
- 需求目录：docs/changes/2026-09-18--allow-zero-discount（plan.md 中的验收标准；S 档为问题描述）
- 档位：L
- 本轮运行配置（由主会话解析后传入）：
## 本轮运行配置
```viktor-checks
...
- 本轮 run_id：20260918201421-96187。你创建的任何资源（容器、临时目录、测试数据前缀）都用它命名，并在**创建后立刻**把标识追加到 `docs/changes/2026-09-18--allow-zero-discount/.check.resources`（每行 `container:<名>` / `dir:<路径>` / `pid:<n>`，只写标识不写命令）。结束前自己清理；超时时包装脚本按这个清单清理，清单外的不会被动。对项目已有的服务只断开连接，不关闭。
- 相关业务口径：`bash /Users/dawson/personWorkSpace/springboot-demo-noinit/.workflow/fe-ai-workflow/scripts/knowledge.sh lookup <AC 涉及的路径或关键词>`（glossary 条目里有判定口径）
...
写入 docs/changes/2026-09-18--allow-zero-discount/check.md：
