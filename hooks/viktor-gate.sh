...
fp="$( { printf '%s\0%s\0' "$WORK" "$checks_blob"; "${G[@]}" diff HEAD -- "${FILES[@]}"; for f in "${FILES[@]}"; do "${G[@]}" ls-files --error-unmatch -- "$f" >/dev/null 2>&1 || { printf '%s\0' "$f"; "${G[@]}" hash-object -- "$TOP/$f" 2>/dev/null; }; done; } | sha_stdin | cut -d' ' -f1 )"
