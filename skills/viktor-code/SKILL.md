...
- **S 档**：直接使用用户描述，没有 plan.md。开始前用一句话声明档位和要做的改动。
...
4. **记录**：在 plan.md 中勾选完成的验收标准或任务（M/L 档）。
...
完成后：S 档若排查中有符合 pitfalls 标准的踩坑（花了明显排查时间，或下一个 Agent 很可能再犯），追加一条到 `docs/knowledge/pitfalls.md`；然后提示下一步使用 /viktor-review，S 档调用时把问题描述作为参数传入。
