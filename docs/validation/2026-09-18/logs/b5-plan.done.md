---
status: done        # draft | confirmed | in-progress | done | archived
tier: L              # S | M | L
stage: done          # plan | code | review | check | ship | done（最近完成或停住的节点）
stage_result: ok     # ok | blocked | error
review_round: 1
base_tree: c92b984cb4a8f3d56166f0590e6ba7cda566490c
verified: {review: 6499c866a7a6, check: 6499c866a7a6, inputs: 7fbd7a404fbc, pending: []}
timing: {plan: 1m, code: 1m, review: 1m, check: 1m, ship: 1m}
created: 2026-09-18
updated: 2026-09-18
---
...
## 验收标准
- [x] AC-1：给定单价 10.00、数量 3、折扣率 0（含 `0`、`0.00` 两种写法），调用 `OrderAmountCalculator.total`，则返回 `0.00`（数值与 scale 均等于 `new BigDecimal("0.00")`）。　证据：core 单测，断言用 `assertEquals(new BigDecimal("0.00"), …)` 同时校验 scale。
  实现：`OrderAmountCalculatorTest.zeroDiscountRateMeansFreeOrder`（RED：IllegalArgumentException「必须在 (0, 1] 之间」；GREEN 后通过）。
- [x] AC-2：折扣率为负数（-0.01）或大于 1（1.01）时，`total` 抛 `IllegalArgumentException`；折扣率 1 仍返回原价。　证据：core 单测覆盖两侧边界。
  实现：`rejectsNegativeDiscountRate`、`rejectsDiscountRateAboveOne`、`fullDiscountRateKeepsOriginalPrice`（原规则下即通过，作为回归保护）。
- [x] AC-3：`POST /orders` 提交 `discountRate: 0`，返回 201，响应 `amount` 为 0.00，数据库中该订单 `amount` 为 0.00。　证据：OrderApiIT 在真实 Postgres（Testcontainers）下断言响应与落库值。
  实现：`OrderApiIT.zeroDiscountRateCreatesFreeOrderAndPersistsZeroAmount`（DTO 未放宽时 RED：期望 201 实际 400；放宽后单独执行通过），完整 verify 留给 check。
- [x] AC-4：`POST /orders` 提交 `discountRate: -0.01` 或 `1.01`，返回 400，不落库。　证据：WebMvc 切片测试断言 400 且 service 未被调用。
  实现：`OrderControllerWebMvcTest.rejectsDiscountRateOutsideZeroToOne`（-0.01 / 1.01）；另加 `acceptsZeroDiscountRate`（0 / 0.00 返回 201，DTO 未放宽时 RED：期望 201 实际 400）。

## 任务
- [x] T1 放宽 core 折扣率下界为含 0，补 0 / 负数 / 大于 1 单测（对应 AC-1、AC-2）
- [x] T2 放宽 `CreateOrderRequest.discountRate` 下界为含 0，补 WebMvc 负数 / 大于 1 返回 400 用例（对应 AC-4；依赖 T1）
- [x] T3 OrderApiIT 新增折扣率 0 创建订单并校验落库 0.00（对应 AC-3；依赖 T1、T2）
