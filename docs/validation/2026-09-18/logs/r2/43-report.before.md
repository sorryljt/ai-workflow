```
━━ ✅ 交付 · 订单创建接口支持幂等键 · L · 合计 10m ━━━━━━━━━━━
改动 9 文件 +281 −8 · 测试 22 passed · review 1 轮 · check ✅ 6 👀 0 ❌ 0 · 待人工 1 项
```

## 待人工确认
| # | 项 | 怎么验 | 期望 |
|---|----|--------|------|
| 1 | 已有数据的库升级（IT 用的是空库） | 对已有 `orders` 表的库执行 `./mvnw -pl app -am spring-boot:run`，再在 psql 里执行 `\d orders` | 出现 `idempotency_key`、`request_hash` 两列和唯一约束 `uk_orders_idempotency_key`；历史行两列都为 NULL |
...
| AC-1 | 带键首次创建时写入键和 SHA-256 指纹；同键并发（本轮新增的证据要求）由 AC-3 的 8 线程用例覆盖 | OrderService.java | OrderApiIT 1 例 + 并发 1 例（真实 Postgres） | ✅ |
...
| AC-5 | 不带键行为不变；键不同但 orderNo 重复仍返回 409 | OrderController.java | OrderApiIT 原有 3 例 + 新增 1 例；WebMvcTest 1 例 | ✅ |
...
- Order.java:14 —— 在已有旧 schema 的库上确认 ddl-auto 能建出列和唯一约束（已列入待人工确认第 1 条）
...
|------|------|--------|-------|------|------|
| 1m（不含等待确认） | 3m | 2m | 1m×3 | 1m×2 | 10m |

## 沉淀的知识
- decisions：订单幂等键存在 orders 表上，不建独立幂等记录表
- pitfalls：OrderService.create 不能加外层 @Transactional，否则并发幂等回查失效
