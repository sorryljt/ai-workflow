...
- 事后单独发起 review 时不再补拍当前状态当基线（会得到空 diff）：工作区有改动用 HEAD，干净用与主干的 merge-base。
- 快照从真实索引复制（保留"已跟踪但被 gitignore"的文件与 intent-to-add）；任一步失败返回非零且不输出，`.review.tree` / `verified` 不采信失败结果。
