...
- app：`CreateOrderRequest.discountRate` 增加 `@Digits(integer = 1, fraction = 4)`，参数校验层返回 400，不进 service。Hibernate Validator 的 `@Digits` 对 BigDecimal 先去尾零再计小数位，与 core 口径一致（code 阶段用 `0.12340` 用例验证）。
...
- `app/src/main/java/com/example/demo/app/order/CreateOrderRequest.java`：`discountRate` 加 `@Digits(integer = 1, fraction = 4)`
...
## 验收标准
- [ ] AC-1：给定单价 10.00、数量 1、折扣率 `0.1235`，`OrderAmountCalculator.total` 返回 `1.24`（10 × 0.1235 = 1.235，HALF_UP）；折扣率 `0.12340` 返回 `1.23`。　证据：core 单测，`assertEquals(new BigDecimal(...), …)` 同时校验 scale。
- [ ] AC-2：折扣率 `0.12345`（5 位）或 `0.000001` 时，`total` 抛 `IllegalArgumentException`；原有 [0, 1] 区间校验不变。　证据：core 单测。
- [ ] AC-3：`POST /orders` 提交 `discountRate: 0.12345`，返回 400，service 未被调用；提交 `0.1235`、`0.12340` 返回 201。　证据：WebMvc 切片测试断言状态码与 `verifyNoInteractions(service)`。
- [ ] AC-4：`POST /orders` 提交单价 10.00、数量 1、`discountRate: 0.1235`，返回 201，响应与数据库中 `amount` 均为 1.24。　证据：OrderApiIT 在真实 Postgres（Testcontainers）下断言响应与落库值。

## 任务
- [ ] T1 core 增加折扣率小数位上限常量与校验，补 4 位 / 5 位 / 尾零单测（对应 AC-1、AC-2）
- [ ] T2 `CreateOrderRequest.discountRate` 加 `@Digits(integer = 1, fraction = 4)`，补 WebMvc 400 / 201 用例（对应 AC-3；依赖 T1）
- [ ] T3 OrderApiIT 新增 4 位折扣率创建订单并校验落库金额（对应 AC-4；依赖 T1、T2）
