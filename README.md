...
只在五处停：plan 确认、缺前置（无 viktor-checks / 无测试框架）、计划偏离或需升档、2 轮复审后仍有 BLOCKING、验收修复一次后仍失败。其余情况不问不等。停下时终端里只有一张卡：需要你拍板的是"待确认"卡，跑不下去的是"需要处理"卡，最后一行列出可以回复什么。停下后人可以手工改代码、在会话里补要求、或者干脆不管。

**续接规则（记住这一条就够）**：任何时候、任何会话，输入 `/viktor-flow` 不带参数，就从自己上次停下的节点继续；同一会话里说"继续"效果相同。别人留下的未完成需求不会被提示。
