...
verified: {review: 7056f88a1a17, check: 7056f88a1a17, inputs: 3115b7b85849, pending: []}
timing: {plan: 1m, code: 3m, review: 2m, check: 1m×3, ship: 1m×2}
created: 2026-09-18
updated: 2026-09-18
---
...
## 验收标准
- [x] AC-1：带 `Idempotency-Key: k1` 首次提交合法订单 → 201，返回订单；库中 1 条且 `idempotency_key = k1`、`request_hash` 非空。证据：OrderApiIT，真实 Postgres 查库；另需覆盖两个并发请求携带同一幂等键的情况
